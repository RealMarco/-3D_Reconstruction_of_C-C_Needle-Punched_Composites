...
# @Improvement: Improve running speed by reducing the times of calling match() function
...
def interpolation(name0,name1,num):                  #inputfolder, outputfolder ,suffix
    ...
    for i in range(1, num):  # 左闭右开，最后一层本来就存在，不需要插入
        img.append(np.zeros(img0.shape, img0.dtype))                     # initialization

    for (x0,y0) in zip(x0array,y0array):
