...
% Step: Triangulation 
% Aim of this program: 
% Improvement: Triangulation with Point Cloud and input data in “.mat ” format.
clf;
clear;
...
%     [row1,col1]=ind2sub([x_range, y_range],locations1);  %Convert linear indices to subscripts    
    ...
    for i = 1:size(locs0,1)   % Traverse in the order of column
        x0 = locs0(i,2);
        y0 = locs0(i,1);
        [point2,point3]=match2(x0,y0,(pic_index+1)*slices_dist,S); % 相邻两层两两 match2()匹配  range(0,12) 
    ...
    for i = 1:size(locs1,1)   % Traverse in the order of column
        x0 = locs1(i,2);
        y0 = locs1(i,1);
        [point2,point3]=match2(x0,y0,pic_index*slices_dist,S); % 相邻两层两两 match2()匹配  range(0,12) M返回两个匹配点的x,y坐标
        point=S(x0,y0,(pic_index+1)*slices_dist);
%         point2 = S(x2,y2,pic_index*slices_dist);
%         point3 = S(x3,y3,pic_index*slices_dist);
        T=[T;[point point2 point3]];
